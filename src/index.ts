...
function checkEmail(params: Config): Promise<Response> {
  return new Promise((resolve, reject) => {
    ...
    const smtpHost: MxRecord =
      addresses?.sort((first, second) => first.priority - second.priority)[0] || defaultMxRecord;
    ...
    const onDataRecieved = (data: string | Buffer): void => {
      if (data.toString()[0] !== '2') quit(false, [commandState.current?.cmd, data].join('\n'));
      else if (commands.filter((x) => x.executed).length === 3) quit(true, '');
      writeNextCommand();
    };
